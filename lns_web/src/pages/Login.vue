<template>
  <div class="login-form">
    ...
    <h2>
      Welcome, please login to access our platform.
      <small>
        <p>or register now, it's painless!</p>
      </small>
    </h2>

    <div class="row">
      <div class="col-md-5">
        <p>This site uses Django (backend rest api), Vue (frontend, black dashboard theme), and Postgres. They are isolated in three Docker containers.</p>
        <p>We've implemented a simple user registration and login system.</p>
      </div>
    </div>
    <form class="form form-default" name="login-form" @submit.prevent="submitLogin">
      <div class="row">
        <div class="col-md-5">
          <base-input label="Username" placeholder="enter username" v-model="username"></base-input>
          <p
            v-if="errors.username && errors.username[0]"
            class="text-danger"
          >{{errors.username && errors.username[0]}}</p>
        </div>
      </div>
      <div class="row">
        <div class="col-md-5">
          <base-input label="Password" type="password" v-model="password"></base-input>
          <p
            v-if="errors.password && errors.password[0]"
            class="text-danger"
          >{{errors.password && errors.password[0]}}</p>
          <p v-if="showWrongUsername" align="left" class="text-danger">Wrong username or password</p>
        </div>
      </div>
      <!-- <div class="row">
        <div class="col-md-5">
      ...
      <div class="row">
        <div class="col-md-5">
          <router-link class="__right" to="/register">
